...
class CoriolisApiViewsTestCase(CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        self._format_fun = views_utils._format_opt

    def _single_view_test(self, fun, expected_result_key):
        format_fun = '%s.%s' % (
            self._format_fun.__module__, self._format_fun.__name__)
        with mock.patch(format_fun) as format_mock:
            result = fun(self._single_response)
            format_mock.assert_called_once_with(self._single_response)
            expected_result = {expected_result_key: format_mock.return_value}
            self.assertEqual(expected_result, result)

    def _collection_view_test(
            self, fun, expected_result_key):
        format_fun = '%s.%s' % (
            self._format_fun.__module__, self._format_fun.__name__)
        with mock.patch(format_fun) as format_mock:
            f_opts = []
            result = fun(self._collection_response)
            for c in self._collection_response:
                format_mock.assert_has_calls([mock.call(c)])
